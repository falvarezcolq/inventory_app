...
<script>
  ...
  export default {
    ...
    methods:{
        async ObtenerMotivoTramite(){
            this.tramite.cod_motivo = '';
            this.tramite.id_tramite = '';
            let data = {params:{...this.params,clas1:this.tramite.cod_tipo_tramite}};
            let result = await api.get(`/getMotivoTramiteByTipo`,data).then((response) => {
                this.motivoTramitesList = response.data.content
            })
        },
        async ObtenerTramite(){     
            this.tramite.id_tramite = '';
            let data = {params:{...this.params,clas1:this.tramite.cod_tipo_tramite,clas3:this.tramite.cod_motivo}};
            let result = await api.get(`/getTramiteByMotivo0/`,data).then((response) => {
