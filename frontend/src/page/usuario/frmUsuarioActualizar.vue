    ...
    <div class="row">
      ...
      <div class="col-md-9">
        ...
        <div class="row form-group mt-2">
          <div class="offset-4 col-md-4">
            ...
            <span class="lb-error" v-if="formError.password">{{ formError.password }}</span>
            <input type="password" class="form-control" v-model='persona.password' :class="{ error: formError.password }" />
          </div>
      </div>
      ...
      <div class="row form-group ">
          <div class="offset-4 col-md-4">
            ...
            <span class="lb-error" v-if="formError.password2">{{ formError.password2 }}</span>
            <input type="password" class="form-control" v-model='persona.password2' :class="{ error: formError.password2 }" />
          </div>
      </div>
...
export default {
  setup() {
    ...
    const Registrar = async () => {
      ...
      if( await Validar()){
        Mensaje.Confirmar("¿Esta seguro de registrar ?<br> ¡Revise a detalle la información antes de registrar!" , ()=>{
           api.post('/setUsuario',persona.value).then(result=>{
              if (result.status === 201 && result.data.codigo === 1) {
                console.log("**Usuario Creado**");
                Mensaje.success_action("Registrado correctamente",()=>{ Login()} );
              }
            }).catch((error)=>{
